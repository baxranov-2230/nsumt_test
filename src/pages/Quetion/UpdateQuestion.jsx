import React, {useEffect, useMemo, useRef} from "react";
import {useFormik} from "formik";
import * as Yup from "yup";
import toast from "react-hot-toast";
import {useNavigate, useParams} from "react-router-dom";
import {useMutation, useQuery} from "@tanstack/react-query";
import {FormControl, InputLabel, MenuItem, Select} from "@mui/material";
import {detailSubjectApi, UpdateSubjectApi} from "../../Api/SubjectApi";
import {detailQuestionApi, UpdateQuestionApi} from "../../Api/QuestionApi.jsx";
import JoditEditor from "jodit-react";

const API_URL = import.meta.env.VITE_TEST_API_URL;

function UpdateQuestion() {
    const {questionId} = useParams();
    const navigate = useNavigate();
    const editorRef = useRef(null);

    const {data} = useQuery({
        queryKey: ["question-detail"],
        queryFn: () => detailQuestionApi(questionId),
    });

    const updateQuestionMutation = useMutation({
        mutationKey: ["update-question"],
        mutationFn: UpdateQuestionApi,
        onSuccess: (data) => {
            toast.success(data.message || "Savol muvaffaqiyatli o'zgartirildi");
        },
        onError: (error) => {
            toast.error(error.message || "Xatolik yuz berdi");
        },
    });
    const formik = useFormik({
        initialValues: {
            text: data?.text || "",
            option_a: data?.option_a || "",
            option_b: data?.option_b || "",
            option_c: data?.option_c || "",
            option_d: data?.option_d || "",
        },
        enableReinitialize: true, //update qilishda qayta render qiladi 1 ta kechmasligi uchun
        validationSchema: Yup.object({
            text: Yup.string().required("!!! To'ldirish shart"),
            option_a: Yup.string().required("!!! To'ldirish shart"),
            option_b: Yup.string().required("!!! To'ldirish shart"),
            option_c: Yup.string().required("!!! To'ldirish shart"),
            option_d: Yup.string().required("!!! To'ldirish shart"),
        }),
        onSubmit: (values) => {
            // setFormData(values);
            const questionData = {
                questionId: questionId,
                text: values.text,
                option_a: values.option_a,
                option_b: values.option_b,
                option_c: values.option_c,
                option_d: values.option_d,

            };
            updateQuestionMutation.mutate(questionData);
        },
    });

    const isSuccess = updateQuestionMutation.isSuccess;

    useEffect(() => {
        if (isSuccess) {
            navigate("/list-question");
        }
    }, [navigate, isSuccess]);

    // 🔹 Jodit sozlamalari
    const config = useMemo(
        () => ({
            readonly: false,
            height: 300,
            toolbarButtonSize: "middle",
            style: {
                table: {
                    border: "1px solid #ccc",
                    "border-collapse": "collapse",
                },
                "td, th": {
                    border: "1px solid #ccc",
                    padding: "5px",
                },
            },
            uploader: {
                url: `${API_URL}/quiz/upload`,
                insertImageAsBase64URI: false,
                imagesExtensions: ["jpg", "png", "jpeg", "gif"],
                method: "POST",
                format: "json",
                prepareData: function (formData) {
                    const file = formData.get("files[0]");
                    formData.delete("files[0]");
                    formData.append("file", file);
                    return formData;
                },
                isSuccess: function (resp) {
                    return resp.file_url !== undefined;
                },
                process: function (resp) {
                    if (resp.file_url) {
                        return {files: [resp.file_url]};
                    }
                    return {files: []};
                },
                defaultHandlerSuccess: function (data) {
                    if (data.files && data.files[0]) {
                        const fileUrl = data.files[0];
                        const ext = fileUrl.split(".").pop().toLowerCase();
                        let html = "";
                        if (["jpg", "jpeg", "png", "gif"].includes(ext)) {
                            html = `<img src="${fileUrl}" alt="image" style="max-width:100%">`;
                        } else {
                            html = `<a href="${fileUrl}" target="_blank">Faylni ochish</a>`;
                        }
                        this.s.insertHTML(html);
                    }
                },
            },
        }),
        []
    );

    return (
        <div className="space-y-6">
            <h2 className="text-2xl font-bold text-gray-800">Savolni o'zgartirish</h2>

            <div className="bg-white rounded-lg shadow">
                <div className="p-4 border-b bg-gray-50  ">
                    <form
                        onSubmit={formik.handleSubmit}
                        className="grid grid-cols-1 gap-3"
                    >

                        <div className="w-full">
                            <label htmlFor="text" className="text-xl mb-2 mt-7">
                                Savol textini kiriting
                            </label>
                            <JoditEditor
                                ref={editorRef}
                                config={config}
                                value={formik.values.text}
                                onChange={(value) => {

                                    formik.setFieldValue("text", value);
                                }}
                            />
                        </div>
                        <div className="w-full">
                            <label htmlFor="option_a" className="text-xl mb-2 mt-7">
                                A variant  matnini kiriting
                            </label>
                            <JoditEditor
                                ref={editorRef}
                                config={config}
                                value={formik.values.option_a}
                                onChange={(value) => {

                                    formik.setFieldValue("option_a", value);
                                }}
                            />
                        </div>
                        <div className="w-full">
                            <label htmlFor="option_b" className="text-xl mb-2 mt-7">
                                B variant  matnini kiriting
                            </label>
                            <JoditEditor
                                ref={editorRef}
                                config={config}
                                value={formik.values.option_b}
                                onChange={(value) => {

                                    formik.setFieldValue("option_b", value);
                                }}
                            />
                        </div>
                        <div className="w-full">
                            <label htmlFor="option_c" className="text-xl mb-2 mt-7">
                                C variant  matnini kiriting
                            </label>
                            <JoditEditor
                                ref={editorRef}
                                config={config}
                                value={formik.values.option_c}
                                onChange={(value) => {

                                    formik.setFieldValue("option_c", value);
                                }}
                            />
                        </div>
                        <div className="w-full">
                            <label htmlFor="option_d" className="text-xl mb-2 mt-7">
                                D variant  matnini kiriting
                            </label>
                            <JoditEditor
                                ref={editorRef}
                                config={config}
                                value={formik.values.option_d}
                                onChange={(value) => {

                                    formik.setFieldValue("option_d", value);
                                }}
                            />
                        </div>
                        <button
                            type="submit"
                            className="focus:outline-none w-full text-white bg-[#3697A5] hover:bg-purple-800 focus:ring-4 focus:ring-purple-300 font-medium rounded-lg text-sm px-5 py-2.5 mb-2 dark:bg-purple-600 dark:hover:bg-purple-700 dark:focus:ring-purple-900"
                        >
                            O'zgartirish
                        </button>
                    </form>
                </div>
            </div>
        </div>
    );
}

export default UpdateQuestion;
