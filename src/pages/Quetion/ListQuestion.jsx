...
function ListQuestion() {
    ...
    const [limitQuestion, setLimitQuestion] = useState(20);
    ...
    const totalPages = Math.ceil(totalCount / limitQuestion);
    const currentPage = Math.floor(offsetQuestion / limitQuestion) + 1;

    const nextPage = () => {
        if (offsetQuestion + limitQuestion < totalCount) setOffsetQuestion((prev) => prev + limitQuestion);
    };

    const prevPage = () => {
        if (offsetQuestion > 0) setOffsetQuestion((prev) => Math.max(prev - limitQuestion, 0));
    };
    ...
    return (
        <div className="space-y-6">
          ...
          Sahifa: {currentPage} / {totalPages || 1}
                    ...
                    <button
                        onClick={nextPage}
                        disabled={offsetQuestion + limitQuestion >= totalCount}
                        className="bg-gray-200 px-4 py-2 rounded disabled:opacity-50"
                    >
                        Keyingi
                    </button>
